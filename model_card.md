...
## 10. Profile Comparison Reflections
...
The generation layer is the part most exposed to misuse. A user could craft a preference
input designed to push unusual, offensive, or harmful content into the system prompt —
for example, embedding instructions in a fake "genre" or "mood" field that tries to
redirect the model's behavior (prompt injection). The current system trusts user-supplied
preference strings directly into the context it sends to the model.
...
2. **Keep the system prompt strictly grounding.** The current prompt already tells the
   model to reason only from retrieved songs and not to reference anything outside the
   list. That is a meaningful guardrail — it limits the blast radius if a bad input gets
   through.
3. **Rate-limit and log every API call.** The existing logging setup captures output per
   request. Extending that to flag unusually long user-supplied strings or anomalous
   outputs would make abuse visible before it scales.
...
being wrong — they only fail to accumulate points. In a small catalog that effect is
visible in almost every run.
...
explicitly forbids the model from listing song titles alone — instead requiring it to cite
